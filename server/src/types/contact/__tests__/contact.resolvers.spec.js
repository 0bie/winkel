...
describe('Resolvers', () => {
  ...
  describe('resolvers:', () => {
    ...
    test('Contact can query by ID', async () => {
      ...
      const result = await resolvers.Query.contact(null, {id: contact._id}, {user: {}});
    ...
    test('Contact can query all contacts', async () => {
      ...
      const result = await resolvers.Query.contacts(null, {}, {user: {}});
    ...
    test('newContact creates a new contact', async () => {
      ...
      const result = await resolvers.Mutation.newContact(null, args, {
        user: {
          role: 'admin',
          _id: mongoose.Types.ObjectId()
        }
    ...
    test('updateContact updates a contact', async () => {
      ...
      const result = await resolvers.Mutation.updateContact(null, args, {
        user: {
          role: 'admin'
        }
      });
    ...
    test('removeContact removes an existing contact', async () => {
      ...
      const result = await resolvers.Mutation.removeContact(null, args, {
        user: {role: 'admin'}
      });
      expect(`${result._id}`).toBe(`${contact._id}`);
    });
